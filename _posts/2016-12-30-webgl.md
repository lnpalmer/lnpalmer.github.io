...
OpenGL is a long-lasting 3D graphics standard that powers the majority of games and graphical applications; [WebGL](https://get.webgl.org/) is a JavaScript API that allows us to use OpenGL inside a web browser environment. At the end of this tutorial we'll have code to render a triangle; on its own this will take a good amount of setup, but from there it doesn't take much more work to build more advanced things.
...
In modern graphics frameworks, 'shaders' are writter to process data; now that we have our vertex data loaded into OpenGL, we need to write a vertex shader and a fragment shader to process it. First, our vertex shader:
